...
public class JsonParser : MonoBehaviour
{
    ...
    public void SaveJsonData(string json, string path, string fileName)
    {
        ...
        FileStream fs = new FileStream(fullPath, FileMode.OpenOrCreate, FileAccess.Write);      // fullPath 경로에 스트림을 있으면 놔두고 없으면 Write 형태로 만든다.
    ...
    public string ReadJsonData(string path, string fileName)
    {
        ...
        if (isDirectoryExist)
        {
            bool isFileExist = File.Exists(fullPath);   // fullPath 경로에 파일이 존재하는 지 bool로 반환

            if (isFileExist)
            {
                // 1. 파일 스트림을 읽기 모드로 연다.
                FileStream fs = new FileStream(fullPath, FileMode.Open, FileAccess.Read);       // fullPath 경로에서 스트림을 Read 모드로 Open 한다.

                // 2. 스트림으로부터 데이터(byte)를 읽어온다.
                StreamReader sr = new StreamReader(fs, Encoding.UTF8);
                readText = sr.ReadToEnd();      // 읽어 온 텍스트!
            }
            else
            {
                readText = "그런 파일 없어욧!";
