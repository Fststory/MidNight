...
public class CameraController : MonoBehaviour
{
    ...
    public float distance = 7.0f;  // 플레이어와 카메라 사이의 거리
    public float height = 20.0f;  // 카메라의 높이 (기존보다 더 높게 설정)
    ...
    public float collisionRadius = 0.5f;  // SphereCast에 사용될 반경

    private float currentX = 0.0f;
    private float currentY = 10.0f;  // 초기 상하 각도를 위쪽으로 설정
    ...
    void Update()
    {
        ...
        currentY = Mathf.Clamp(currentY, -10f, 70f);  // 상하 회전 각도 제한
    ...
    void HandleCameraPosition()
    {
        ...
        transform.position = player.position + direction + Vector3.up * height * 0.5f;  // 카메라를 더 위로 위치시킴
    ...
    void HandleCameraCollision()
    {
        ...
        if (Physics.SphereCast(player.position, collisionRadius, rayDir, out RaycastHit hit, targetDistance, cameraCollision))
        {
            // 충돌 발생 시 카메라를 충돌 지점으로 이동, 최소 거리를 유지
            float distanceToHit = Mathf.Clamp(hit.distance, minDistance, targetDistance);
            transform.position = player.position + rayDir.normalized * distanceToHit + Vector3.up * height * 0.5f;
        }
        else
        {
            // 충돌이 없으면 원래의 위치로
            transform.position = player.position + rayDir.normalized * targetDistance + Vector3.up * height * 0.5f;
        }
    }
}
